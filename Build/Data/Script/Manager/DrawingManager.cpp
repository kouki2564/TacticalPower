...
void DrawingManager::Draw()
{
	for (auto t : m_models)
	{
		...
		/* シェーダ予定地 */
		//for (int i = 0; i < MV1GetTriangleListNum(t.second.handle); i++)
		//	{
		//		int shaderType = MV1GetTriangleListVertexType(t.second.handle, i);
		//		m_tShader.SetShader(shaderType);

		//		// m_tShader.SetShaderField(shaderType);

		//		MV1DrawTriangleList(t.second.handle, i);
		//		m_tShader.EndShader();
		//	}
