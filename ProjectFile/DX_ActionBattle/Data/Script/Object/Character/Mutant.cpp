...
void Mutant::Init(std::string name, VECTOR pos, VECTOR dir)
{
	...
	// モデルデータ設定
	auto handle = MV1LoadModel("Data/Model/Boss_1.mv1");
	m_name = name;
	m_pos = VGet(0, 0, 100);
	m_dir = VGet(0, 0, -1);
	...
	// パラメータ設定(マジックナンバー)
	m_parameter.SetPalameter(500, 10, 10, 10);
...
void Mutant::SetCameraDir(VECTOR cameraDir)
{
}

void Mutant::SetDamagePoint(float damagePoint)
{
	// 防御値分の計算
...
bool Mutant::GetIsHitAttack(Collider& colOther)
{
	...
	if (colOther.isOnDamage)
	{
		...
		bool isHitR = chacker.GetIsCollision(m_hitCols["RightHand"], colOther);
		bool isHitL = chacker.GetIsCollision(m_hitCols["LeftHand"], colOther);
		bool isHitBody = chacker.GetIsCollision(m_collider, colOther);
		bool isResHit = false;
