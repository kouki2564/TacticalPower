...
void Magic::SetShot(int& shotHandle, VECTOR shotDir, float shotSize, int shotNum)
{
	...
	if (shotNum == 1)
	{
		...
		// エフェクトの登録
		EffectManager::Instance().RegisterEffect(name, LoadEffekseerEffect("Data/Effect/PlayerShot.efkefc", 1.0f), m_spherePos, shotDir);
		Shot* temp = new Shot;
	...
	else
	{
		for (int num = 0; num < shotNum; num++)
		{
			...
			auto name = "Pshot_" + std::to_string(m_shotNum);
			// エフェクトの登録
			auto handle = LoadEffekseerEffect("Data/Effect/PlayerShot.efkefc", 20.0f);
			EffectManager::Instance().RegisterEffect(name, handle, m_spherePos, shotDir);
			Shot* temp = new Shot;
			// 分散させる角度
			...
			temp->SetUp(m_shotNum, shotHandle, m_spherePos, m_targetPos, kShotTime, shotDir, resDir, shotSize);
			m_shots.push_back(temp);
			Collider colTemp;
			colTemp.isChackOther = true;
			colTemp.SetSphere(m_spherePos, -1, 5);
